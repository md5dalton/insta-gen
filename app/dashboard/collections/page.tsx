...
export default () => {
    ...
    const handleSaveConfiguration = async () => {
        ...
        try {
            ...
            showFeedback(
                `Configuration updated for "${selectedEntity.data.name || selectedEntity.data.displayName}"`
            )
            await loadData()
        } catch (err: any) {
    ...
    const handleSoftDelete = async () => {
        ...
        try {
            ...
            showFeedback(
                `"${selectedEntity.data.name || selectedEntity.data.displayName}" marked as soft-deleted.`
            )
    ...
    const handleRestore = async () => {
        ...
        try {
            ...
            showFeedback(
                `"${selectedEntity.data.name || selectedEntity.data.displayName}" restored.`
            )
            await loadData()
        } catch (err: any) {
    ...
    const effectiveProfileName = () => {
        ...
        if (selectedEntity.type === "user" && selectedEntity.parentData?.processingProfileId) {
            return (
                profiles.find((p) => p.id === selectedEntity.parentData.processingProfileId)
                    ?.name || "Direct"
            )
        }
        if (
                        ...
                        <>
                                ...
                                <div>
                                    ...
                                    <h2 className="text-xl font-bold text-white mt-0.5">
                                        {selectedEntity.data.name ||
                                            selectedEntity.data.displayName}
                                    </h2>
                                    <p className="text-xs font-mono text-slate-400 mt-1">
                                        Path:{" "}
                                        {selectedEntity.data.path || selectedEntity.data.username}
                                    </p>
                                </div>
                            ...
                            <div className="space-y-3 pt-2 border-t border-slate-800">
                                <AccessPolicyEditor
                                    ...
                                    allowInherit={selectedEntity.type !== "root"}
                                    inheritedVisibility={
                                        selectedEntity.parentData?.visibility ||
                                        selectedEntity.rootData?.visibility ||
                                        "ALL_USERS"
                                    }
                                    ...
                                    parentName={
                                        selectedEntity.parentData?.name ||
                                        selectedEntity.rootData?.name
                                    }
                                />
                            </div>
