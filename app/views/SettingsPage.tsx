    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              The canonical storage root directory path scanned by the media discovery engine.
            </p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-amber-950/40 border border-amber-800/80 text-xs text-amber-200 flex items-start gap-3">
          ...
          <div className="space-y-1">
            <strong className="font-semibold text-amber-300 block">Critical Path Notice</strong>
            <p className="leading-relaxed">
              <strong>Changing the media root does not move or copy existing media.</strong> The system will point
              discovery to the specified path without relocating files on the underlying filesystem.
            </p>
          </div>
        </div>
      ...
      <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Profiles determine which renditions (HLS, Feed Images, LQ fallbacks) are mandatory for media items.
              </p>
            </div>
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800 text-xs text-slate-300 flex items-start gap-2.5">
          ...
          <span>
            <strong>Mandatory Thumbnail Enforcement:</strong> Every processing profile strictly requires a thumbnail
            asset. Additional renditions (Feed Image, HLS stream, Low-Quality 720p/480p) are configurable per profile.
          </span>
        </div>
      ...
      <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Real-time status of backend processing services</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
          <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-1">
            ...
            <p className="text-[11px] text-slate-500">Watching filesystem for new uploads</p>
          </div>
          ...
          <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-1">
            ...
            <p className="text-[11px] text-slate-500">FFmpeg transcode and HLS packagers</p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold text-white">Database & Prisma Schema</h2>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-cyan-950 text-cyan-300 border border-cyan-800">
                  prisma/schema.prisma
                </span>
              </div>
              <p className="text-xs text-slate-400 mt-0.5">
                Complete relational schema with models, enum definitions, indexes, and cascading constraints.
              </p>
            </div>
          </div>
        ...
        {activeSchemaTab === 'models' ? (
          <div className="space-y-4">
                ...
                >
                  ...
                  <div className="pt-2 border-t border-slate-800/60 space-y-1">
                    {m.fields?.slice(0, 4).map((f: string) => (
                      <div key={f} className="text-[10px] font-mono text-slate-300 truncate">
                        • {f}
                      </div>
                    ))}
                    {m.fields?.length > 4 && (
            ...
            <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 text-xs text-slate-400 flex items-start gap-2.5">
              ...
              <div className="space-y-1">
                <span className="font-semibold text-slate-200 block">Hierarchy Cascade & Inheritance</span>
                <p className="text-[11px] text-slate-400">
                  Relationships strictly enforce <code className="text-cyan-300 font-mono">onDelete: Cascade</code> across 
                  RootCollection &rarr; Collection &rarr; MediaUser &rarr; MediaItem &rarr; MediaAsset, with soft-deletion inheritance and deterministic policy fallback.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="relative rounded-xl border border-slate-800 bg-slate-900/90 overflow-hidden">
              <div className="px-4 py-2 bg-slate-950 border-b border-slate-800 flex items-center justify-between text-xs text-slate-400">
                ...
                <span className="text-[11px] text-slate-500">PostgreSQL / MySQL / SQLite compatible</span>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="p-2.5 rounded-xl bg-slate-900 border border-emerald-900/60 flex items-center justify-between">
                    <div>
                      <span className="font-semibold text-slate-200 block">Thumbnail</span>
                      <span className="text-emerald-400 text-[11px] block">Mandatory for all media</span>
                    </div>
                    <span className="text-xs text-emerald-400 font-bold">Locked ✓</span>
                  </div>

                  <label className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between cursor-pointer">
                    <div>
                      <span className="font-semibold text-slate-200 block">Feed Image (1080p WebP)</span>
                      <span className="text-slate-400 text-[11px] block">Optimized for feed scrolling</span>
                    </div>
                    <input
                      type="checkbox"
                      checked={feedImage}
                  ...
                  <label className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between cursor-pointer">
                    <div>
                      <span className="font-semibold text-slate-200 block">HLS Stream (Multi-bitrate)</span>
                      <span className="text-slate-400 text-[11px] block">Adaptive bitrate video streaming</span>
                    </div>
                    <input
                      type="checkbox"
                      checked={hls}
                  ...
                  <label className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between cursor-pointer">
                    <div>
                      <span className="font-semibold text-slate-200 block">Low Quality (720p / 480p fallback)</span>
                      <span className="text-slate-400 text-[11px] block">Low-bandwidth fallback</span>
                    </div>
                    <input
                      type="checkbox"
                      checked={lowQuality}
