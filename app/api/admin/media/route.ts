...
export async function GET(request: Request) {
  ...
  if (q.rootCollectionId) results = results.filter((m) => m.rootCollectionId === q.rootCollectionId);
  ...
  return NextResponse.json({ items: paginated, total, page: pageNum, limit: limitNum, totalPages });
}
...
export async function POST(request: Request) {
  ...
  if (!name || !userId || !collectionId || !rootCollectionId) {
    return NextResponse.json({ error: 'name, userId, collectionId, and rootCollectionId are required' }, { status: 400 });
  }
  ...
  const newMedia: any = {
    ...
    likesCount: 0,
    savesCount: 0,
    previewUrl: previewUrl || 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?w=800&auto=format&fit=crop&q=80',
