...
export async function GET(request: Request) {
    ...
    try {
        const [totalFiles, readyFiles, processingFiles, errorFiles, totalRenditions, readyRenditions, missingRenditions, failedRenditions, mediaSum, assetSum, totalRootCollections, totalCollections, totalMediaUsers, recentActivity] = await Promise.all([
        ...
        const stats = {
            ...
            recentActivity: recentActivity.map((a: any) => ({ id: a.id, type: a.type, title: a.title, description: a.description, timestamp: a.timestamp })),
        }
