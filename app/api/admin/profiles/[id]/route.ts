...
export async function PUT(request: any, context: any) {
    ...
    if (Array.isArray(renditions)) {
        profile.renditions = renditions.filter((value: unknown) => value === "THUMBNAIL" || value === "FEED_IMAGE" || value === "HLS")
        if (!profile.renditions.includes("THUMBNAIL")) profile.renditions.unshift("THUMBNAIL")
    }
