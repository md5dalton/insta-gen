...
export const ProcessingPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'ALL' | 'NEW' | 'NEEDS_PROCESSING' | 'PROCESSING' | 'READY' | 'FAILED'>('NEEDS_PROCESSING');
  ...
  const handleBulkProcess = async () => {
    ...
    try {
      ...
      showFeedback(`Processing completed for ${res.affectedCount} media items (${res.skippedCount} skipped).`);
      setSelectedIds([]);
      loadData();
    } catch (err: any) {
  ...
  const tabs = [
    { id: 'NEEDS_PROCESSING' as const, label: 'Needs Processing', icon: <AlertTriangle className="w-3.5 h-3.5" /> },
    { id: 'NEW' as const, label: 'New Ingested', icon: <Sparkles className="w-3.5 h-3.5" /> },
    { id: 'FAILED' as const, label: 'Failed Transcodes', icon: <ShieldAlert className="w-3.5 h-3.5" /> },
    { id: 'PROCESSING' as const, label: 'Active Jobs', icon: <Loader2 className="w-3.5 h-3.5" /> },
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Processing Engine</h1>
          <p className="text-sm text-slate-400 mt-1">
            Enforce rendition profiles, verify mandatory thumbnails, and resolve missing assets.
          </p>
        </div>
      ...
      <div className="bg-indigo-950/40 border border-indigo-800/60 rounded-2xl p-4 sm:p-5 flex items-start gap-3.5 text-xs text-indigo-200">
        ...
        <div className="space-y-1">
          ...
          <p className="text-indigo-300 leading-relaxed">
            <strong>Thumbnails are mandatory for 100% of media items.</strong> Additional renditions (Feed Image, HLS
            streams, Low-Quality fallbacks) are derived from the effective processing profile ({' '}
            <code className="font-mono text-[11px] text-white">Media → User → Collection → Root Collection → Default</code>
            ). Media items lacking any required rendition automatically flag as <em>Needs Processing</em>.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {loading ? (
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-16 text-center text-slate-400 space-y-2">
            ...
            <h3 className="text-sm font-semibold text-slate-200">No media items in this state</h3>
            <p className="text-xs text-slate-500">
              All items in the library meet their assigned processing profile requirements.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-3.5 flex-1">
                    ...
                    <div className="space-y-1.5 flex-1">
                      ...
                      <div className="text-[11px] text-slate-400">
                        {item.rootCollectionName} → {item.collectionName} → @{item.userName}
                      </div>
                      ...
                      {item.processingError && (
                        <div className="mt-2 p-2.5 rounded-lg bg-rose-950/60 border border-rose-800/60 text-[11px] text-rose-300 font-mono">
                          <strong>Stage: {item.processingError.stage}</strong> — {item.processingError.message}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="lg:w-72 bg-slate-900/80 p-3 rounded-xl border border-slate-800 text-xs space-y-2 shrink-0">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-center justify-between text-[11px]">
                        ...
                        {item.assets.some((a) => a.type === 'THUMBNAIL' && a.status === 'READY') ? (
                      ...
                      {/* Feed Image */}
                      {item.effectivePolicy?.requiredAssets.includes('FEED_IMAGE') && (
                        <div className="flex items-center justify-between text-[11px]">
                          <span className="text-slate-300 flex items-center gap-1.5">
                            <span className="w-1.5 h-1.5 rounded-full bg-indigo-400"></span>
                            Feed Image
                          </span>
                          {item.assets.some((a) => a.type === 'FEED_IMAGE' && a.status === 'READY') ? (
                            <span className="text-emerald-400 font-medium">✓ Ready</span>
                          ) : (
                            <span className="text-amber-400 font-bold">✗ Missing</span>
                          )}
                      ...
                      {/* HLS */}
                      {item.effectivePolicy?.requiredAssets.includes('HLS') && (
                        <div className="flex items-center justify-between text-[11px]">
                          <span className="text-slate-300 flex items-center gap-1.5">
                            <span className="w-1.5 h-1.5 rounded-full bg-purple-400"></span>
                            HLS Stream
                          </span>
                          {item.assets.some((a) => a.type === 'HLS' && a.status === 'READY') ? (
                            <span className="text-emerald-400 font-medium">✓ Ready</span>
                          ) : (
                            <span className="text-amber-400 font-bold">✗ Missing</span>
                          )}
                      ...
                      {/* Low Quality */}
                      {item.effectivePolicy?.requiredAssets.includes('LOW_QUALITY') && (
                        <div className="flex items-center justify-between text-[11px]">
                          <span className="text-slate-300 flex items-center gap-1.5">
                            <span className="w-1.5 h-1.5 rounded-full bg-amber-400"></span>
                            Low Quality (720p)
                          </span>
                          {item.assets.some((a) => a.type === 'LOW_QUALITY' && a.status === 'READY') ? (
                            <span className="text-emerald-400 font-medium">✓ Ready</span>
                          ) : (
                            <span className="text-amber-400 font-bold">✗ Missing</span>
                          )}
