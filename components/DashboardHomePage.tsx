...
interface DashboardHomePageProps {
  ...
  onNavigate: (tab: 'dashboard' | 'media' | 'processing' | 'collections' | 'access' | 'settings') => void;
...
export const DashboardHomePage: React.FC<DashboardHomePageProps> = ({
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div>
        <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Library Scope</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          >
            ...
            <p className="text-xs text-slate-500 mt-1">Active files discovered across collections</p>
          </div>
          ...
          >
            <div className="flex items-center justify-between text-slate-400 mb-2">
              ...
              <span className="text-xs font-mono text-emerald-400 font-medium">JPEG / PNG / WebP</span>
            </div>
            <div className="text-2xl sm:text-3xl font-extrabold text-white">
          ...
          >
            <div className="flex items-center justify-between text-slate-400 mb-2">
              ...
              <span className="text-xs font-mono text-sky-400 font-medium">MP4 / MOV / HLS</span>
            </div>
            <div className="text-2xl sm:text-3xl font-extrabold text-white">
              {stats?.videoCount?.toLocaleString() || 0}
            </div>
            <p className="text-xs text-slate-500 mt-1">Reels, clips, and stream ingest</p>
          </div>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider">What Needs My Attention?</h2>
          <span className="text-xs text-indigo-400 font-medium">Operational Action Items</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            <div className="flex items-center justify-between text-purple-400 mb-2">
              ...
              <span className="text-[11px] font-semibold text-purple-300">Ingested</span>
            </div>
            <div className="text-2xl font-bold text-white mt-2">{stats?.newMediaCount || 0}</div>
          ...
          >
            ...
            <div className="text-2xl font-bold text-white mt-2">{stats?.needsProcessingCount || 0}</div>
            <div className="text-xs font-semibold text-amber-200 mt-1">Need Processing</div>
            <p className="text-[11px] text-slate-400 mt-1 leading-relaxed">
          ...
          >
            ...
            <div className="text-2xl font-bold text-white mt-2">{stats?.processingFailuresCount || 0}</div>
            <div className="text-xs font-semibold text-rose-200 mt-1">Processing Failures</div>
            <p className="text-[11px] text-slate-400 mt-1 leading-relaxed">
          ...
          >
            <div className="flex items-center justify-between text-slate-400 mb-2">
              ...
              <span className="text-[11px] font-semibold text-slate-400">Soft-Deleted</span>
            </div>
            <div className="text-2xl font-bold text-white mt-2">{stats?.markedDeletedCount || 0}</div>
            <div className="text-xs font-semibold text-slate-300 mt-1">Marked Deleted</div>
            <p className="text-[11px] text-slate-400 mt-1 leading-relaxed">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-3">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Administrative Modules</h2>
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl divide-y divide-slate-800/80 overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-xs font-semibold text-white block">Processing Engine</span>
                  <span className="text-[11px] text-slate-400 block">Manage renditions, HLS, and feed profiles</span>
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-xs font-semibold text-white block">Collections Hierarchy</span>
                  <span className="text-[11px] text-slate-400 block">RootCollection → Collection → User policies</span>
                </div>
              </div>
              <ChevronRight className="w-4 h-4 text-slate-500 group-hover:text-white group-hover:translate-x-0.5 transition-all" />
            </button>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-xs font-semibold text-white block">Access & Users</span>
                  <span className="text-[11px] text-slate-400 block">Manage VIEW, DOWNLOAD, and restrictions</span>
                </div>
              </div>
              <ChevronRight className="w-4 h-4 text-slate-500 group-hover:text-white group-hover:translate-x-0.5 transition-all" />
            </button>
        ...
        <div className="lg:col-span-7 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Recent System Activity</h2>
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 shadow-xl space-y-3 max-h-[300px] overflow-y-auto">
            {stats?.recentActivity && stats.recentActivity.length > 0 ? (
              stats.recentActivity.map((act) => {
                const badgeColor = {
                  DISCOVERY: 'bg-purple-950 text-purple-400 border-purple-800',
                  PROCESSED: 'bg-emerald-950 text-emerald-400 border-emerald-800',
                  FAILED: 'bg-rose-950 text-rose-400 border-rose-800',
                  POLICY_CHANGE: 'bg-indigo-950 text-indigo-400 border-indigo-800',
                  ...
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.2 rounded border ${badgeColor}`}>
                          {act.type.replace('_', ' ')}
                        </span>
              ...
              <div className="text-center py-6 text-xs text-slate-500">No recent activity logged</div>
            )}
          </div>
        </div>
      </div>
