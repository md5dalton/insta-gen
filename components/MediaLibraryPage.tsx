...
export const MediaLibraryPage: React.FC = () => {
    ...
    const [selectedDeletionStatus, setSelectedDeletionStatus] = useState<
        "ACTIVE" | "DELETED" | "ALL"
    >("ACTIVE")
    ...
    const handleBulkProcess = async () => {
        try {
            ...
            showFeedback(
                `Triggered processing for ${res.affectedCount} media items (${res.skippedCount} skipped).`
            )
            setSelectedIds([])
            loadMedia()
        } catch (err: any) {
    ...
    const handleBulkMarkDeleted = async () => {
        try {
            ...
            showFeedback(
                `Marked ${res.affectedCount} media items as soft-deleted. Awaiting filesystem cleanup.`
            )
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-400 mt-1">
                        Browse discovered media, inspect processing renditions, and manage
                        overrides.
                    </p>
                </div>
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center justify-between text-xs text-slate-400 px-1">
                    <div className="flex items-center gap-3">
                        ...
                        <span>•</span>
                        <span>
                            Total: <strong>{total.toLocaleString()}</strong> media items
                        </span>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="bg-slate-950 border border-slate-800 rounded-2xl p-16 text-center text-slate-400 space-y-3">
                        ...
                        <h3 className="text-sm font-semibold text-slate-200">
                            No media matched your filters
                        </h3>
                        <p className="text-xs text-slate-500 max-w-sm mx-auto">
                            Try adjusting your search keywords, clearing status filters, or
                                    ...
                                    <div className="p-3.5 flex-1 flex flex-col justify-between space-y-2.5">
                                        ...
                                        <div className="bg-slate-900/60 px-2 py-1 rounded text-[10px] text-slate-400 flex items-center justify-between">
                                            ...
                                            {item.processingProfileId && (
                                                <span className="text-indigo-400 font-semibold uppercase text-[9px]">
                                                    Override
                                                </span>
                                            )}
                                        </div>
                                    ...
                                    {mediaList.map((item) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <td className="p-3.5">
                                                    <div
                                                        className="w-12 h-12 rounded-lg overflow-hidden shrink-0 cursor-pointer"
                                                        onClick={() => setInspectingMedia(item)}
                                                    >
                                                        <MediaPreview
                                                            url={
                                                ...
                                                <td className="p-3.5">
                                                    <span className="font-semibold text-slate-300 block">
                                                        {item.effectivePolicy?.profile.name ||
                                                            "Direct"}
                                                    </span>
            ...
            <ConfirmDialog
                ...
                description={
                    <span>
                        You are about to mark <strong>{selectedIds.length} media items</strong> as
                        soft-deleted. The database records will be flagged as deleted and will await
                        eventual filesystem cleanup. Files are never immediately destroyed by this
                        dashboard.
                    </span>
                }
            />
