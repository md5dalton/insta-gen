          ...
          >
            <div>
              <div className="flex items-center gap-2">
                <span className="text-sm font-semibold text-slate-900">Inherit from parent</span>
                <span className="text-[11px] font-medium text-slate-500 bg-slate-100 px-2 py-0.5 rounded">
                  Effective: {inheritedProfileName}
                </span>
              </div>
              <p className="text-xs text-slate-500 mt-0.5">
                Automatically uses the processing profile assigned to the parent hierarchy level.
              </p>
            </div>
            {(value === null || value === undefined) && <Check className="w-4 h-4 text-indigo-600 shrink-0 mt-0.5" />}
          </button>
        )}
