...
export class FFprobe {
    ...
    async duration(inputPath: string): Promise<number> {
        ...
        const raw =
            ...
            probe.streams.find((item) => item.codec_type === "video")?.duration
        if (!raw) {
    ...
    async bitrate(inputPath: string): Promise<string | undefined> {
        ...
        return (
            ...
            probe.streams.find((item) => item.codec_type === "video")?.bit_rate
        )
    }
    ...
    async audio(
        inputPath: string
    ): Promise<{ codec?: string; channels?: number; sampleRate?: number } | undefined> {
        const probe = await this.probe(inputPath)
